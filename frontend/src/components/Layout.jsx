...
import { LayoutDashboard, CheckSquare, Building2, LogOut, Menu, X, FileText, ChevronRight } from 'lucide-react';
  ...
  const NavLink = ({ item, onClick }) => {
    ...
    return (
      ...
      >
        ...
        <Icon className={`w-4.5 h-4.5 flex-shrink-0 ${isActive ? item.color : 'text-muted-foreground group-hover:text-foreground'} transition-colors`} />
        <span className={`font-medium text-sm ${isActive ? 'text-foreground' : ''}`}>{item.label}</span>
