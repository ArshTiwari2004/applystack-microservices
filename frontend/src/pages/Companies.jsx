...
export default function Companies() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        localStorage.setItem(
          "last_not_applied_count",
          notAppliedCount.toString(),
        );
      }
    } catch (error) {
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      doc.text(
        "ApplyStack - One stop job application tracking system",
        pageWidth / 2,
        20,
        { align: "center" },
      );
      ...
      const appliedCount = filteredJobs.filter(
        (j) => j.applied === "yes",
      ).length;

      doc.setFontSize(11);
      doc.text(
        `Total Companies: ${totalCompanies} | Total Applications: ${totalApplications} | Applied: ${appliedCount}`,
        pageWidth / 2,
        36,
        { align: "center" },
      );
      ...
      autoTable(doc, {
        ...
        didParseCell: function (data) {
          if (data.section === "body") {
            ...
            // Green statuses
            if (value === "Yes" || value === "Offer" || value === "Done") {
            ...
            // Yellow statuses
            if (value === "In Progress" || value === "Waiting") {
  ...
  return (
    <Layout>
      <div className="space-y-4 sm:space-y-6" data-testid="companies-page">
        ...
        {showFilters && (
          <div
            className="p-4 rounded-lg bg-card border border-white/5"
        ...
        {statusCounts.not_applied > 0 &&
          statusFilter === "all" &&
          !reminderDismissed && (
            <div
              className="p-4 sm:p-6 rounded-lg bg-gradient-to-r from-warning/10 to-destructive/10 border border-warning/30 relative"
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pr-8">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    ...
                    {statusCounts.not_applied === 1 ? "company" : "companies"}{" "}
                    waiting for your application. Don't miss out on these
                    opportunities!
                  </p>
                </div>
                <Button
